...
And ('I verify default value and enter new start origin date',()=> {
    ...
    incidentlocators.INC_STARTDATE_SR().should('be.empty')
    incidentlocators.INC_STARTDATE_SR().click({force:true}).type(incidentorigindate,{force: true})
})

And ('I verify default value and enter new tenure', ()=> {
    ...
    incidentlocators.TENURE_TAB_SR().should('have.text', 'Undetermined×')
...
And ('I verify default value and threat potential in <2 hours',()=>{
    incidentlocators.THREATS_TAB_SR().click({force: true}).waitForLoadingSpinner()
    incidentlocators.THREAT_POTENTIAL_SR().should('have.text','Unknown')
...
And ('verify Alert modal window',()=>{
    ...
    //update ICONSUP-3677, default alert level is being set. therfore
    //incidentlocators.ALERTLEVEL_REQTEXT().should('contain.text','Current Alert Level is required.')
    incidentlocators.ALERTLEVEL_REQTEXT().should('contain.text','Selected Alert Level: A')
...
And ('I verify default value and select new Incident Controller',()=>{
    incidentlocators.INCCONTROLLER_SRE_TAB().click({force: true}).waitForLoadingSpinner()   //Expand Command/control/comms
    incidentlocators.INCCONTROLLER_SRE().should('have.value', 'RFS')
    incidentlocators.INCCONTROLLER_SRE().clear().type('TeamC', {force: true})

})

And ('I verify default value and select new Lead Agency',()=> {

    incidentlocators.INCLEAD_AGENCY_SRE().should('have.value', 6)
...
And ('I enter incident name in the field on create full incident page - other types',()=>{

    incidentlocators.INC_NAME_OTHER_TYPE().type('FullIncidentAutomationOther',{force: true})
})



And ('I select Other Fire Type on create full incident page', ()=>{

    incidentlocators.INC_OTHERFIRE_TYPE().select('Garage fire',{force:true})
...
And ('I verify fire alert level and fire class are not present in sitrep',()=>{
    incidentlocators.INCCONTROLLER_SRE_TAB().click({force: true}).waitForLoadingSpinner()   //Expand Command/control/comms
    incidentlocators.FIRE_CLASS_SR().should('not.exist')
    incidentlocators.ACTION_TAB_SR().click({force: true}).waitForLoadingSpinner()
    incidentlocators.ALERTLEVEL_REQTEXT().should('not.exist')
    incidentlocators.SETALERTBTTN_SR().should('not.exist')
})
...
And ('I select Non-fire Incident to incident creation',()=> {
    commonlocators.INC_NONFIRE_TYPE().scrollIntoView().select('81')
...
And ('I enter location to incident creation - Call out',()=>{
    ...
    incidentlocators.INC_LOCATION_CALLOUT1().type('WOOLLAHRA ',{force: true})
    incidentlocators.INC_LOCATION_CALLOUT2().click({force: true}).wait(4000) // Tap On ZOOM BUTTON

    loc_address = '12 SABER ST, WOOLLAHRA'
    incidentlocators.INCIDENT_LOCATION().type(loc_address,{force: true}).wait(5000).type('{downArrow}').type('{enter}')
    incidentlocators.INC_LOCATION_CALLOUT2().click({force: true})    // Tap On ZOOM BUTTON
})
...
///>>>>>>>>>>>>>>>>>>>>>>>>SET TO OUT >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
 ...
 And ('I write Incident name to incident creation -settoout',()=> {
     ...
     incidentlocators.INC_NAME_BUSH().type('AutomationBushfireShell-settoout',{force: true})
 ...
 And ('I enter location to incident creation - bushfireshell -settoout',()=> {
    ...
    incidentlocators.INC_LOCATION_BUSH1().type('',{force: true})
    incidentlocators.INC_LOCATION_BUSH2().click({force: true}).wait(4000)

    loc_address = '525 SWIFT ST, ALBURY'
    incidentlocators.INCIDENT_LOCATION().type(loc_address,{force: true}).wait(5000).type('{downArrow}').type('{enter}')
    incidentlocators.ZOOMBUTTON_INC_MAP().click({force: true})
 })
...
// "RFS_callTime" "STOP_MESSAGE_TIME" "INC_TYPE" "REASON_FOR_TYPE_CHANGE"
...
And ('I edit Incident and set to out -settoout',()=> {
    ...
    incidentlocators.AGENCY_CALL_RECEIVED().scrollIntoView().type(RFSreceivedcall, {force: true})
...
//Updating the test case for a warning message instead of Success under ICONSUP-3613
//adding new step
...
And ('I assert for Warning -settoout', ()=> {

    cy.waitUntil(()=>incidentlocators.INC_WARNING_SETTOIUT()
    .contains('Warning!')).should('exist')
})
...
And ('I can only edit agency received time after settoout and cannot change the incident type',()=>{
    ...
    incidentlocators.AGENCY_CALL_RECEIVED().scrollIntoView().type(RFSreceivedcallupdated, {force: true})
...
//Updated scenario implementation after master incident change. ICONSUP-3850
...
And ('I look for the incidents to be merged',()=> {
    ...
    incidentlocators.INC_SEARCHMERGE().type('AutomationBush',{force: true})
...
And ('I select incidents and click on merge',()=> {
    ...
    incidentlocators.ATIFINC_SELECTED1().click({force : true})

    // incidentlocators.ATIFINC_SELECTED2().click({force : true})
...
And ('I select primary Incidents',()=> {
    incidentlocators.INC_MASTER().click({force: true})
    incidentlocators.INC_NON_MASTER().click({force: true})

    incidentlocators.INC_MASTER().parent().siblings().find('span').then(($fields)=>{
        let info_masterINC0 = $fields.eq(0).text()
        cy.wrap(info_masterINC0).as('info_Master0')
        let info_masterINC1 = $fields.eq(1).text()
        cy.wrap(info_masterINC1).as('info_Master1')
        let info_masterINC2 = $fields.eq(2).text()
        cy.wrap(info_masterINC2).as('info_Master2')
        let info_masterINC3 = $fields.eq(3).text()
        cy.wrap(info_masterINC3).as('info_Master3')

    })

    incidentlocators.INC_NON_MASTER().parent().siblings().find('span').then(($fields)=>{
        let info_non_masterINC = $fields.eq(3).text()
        cy.wrap(info_non_masterINC).as('info_Non_MasterRef')
    })

    incidentlocators.BTN_INC_MERGE().click({force: true})
...
And ('I verify with the merged details',()=> {
    ...
    cy.get('@info_Master3').then((primaryRef)=>{
        incidentlocators.INC_SUMMARYDETAILS().invoke('text')
            .should('contain', primaryRef)
    })
    cy.get('@info_Non_MasterRef').then((secRef)=>{
        incidentlocators.INC_SUMMARYDETAILS().invoke('text')
            .should('contain', secRef)
    })
...
And ('I select Advanced incident search',()=> {
    ...
    // incidentlocators.map_View().then(($coordinate)=>{

    //     let primary_merge = $coordinate.getAttribute
})
...
And ('Edit incident from bushfire to non bushfire',()=> {
    ...
    incidentlocators.AGENCY_CALL_RECEIVED().scrollIntoView().type(agency_recievedcall, {force : true})
    ...
    cy.wait(5000)
    incidentlocators.INC_OTHERFIRE_TYPE().select("Chimney fire")
...
And('I create full incident with pending sitrep',()=>{
    ...
    incidentlocators.INCIDENT_NAME().then(($nameField)=>{
        const nameField = $nameField
        const timeStamp = Date.now().toString()
        const name = 'ICONSUPT425-428:Sitrep Pending- ' + timeStamp
    ...
    cy.wrap('Armidale').as('lga')
    // I enter incident location on create full incident page

    incidentlocators.ZOOMBUTTON_INC_MAP().then(($el)=>{
    ...
    incidentlocators.ZOOMBUTTON_INC_MAP().click({force: true}).wait(4000)

    loc_address = 'COUNTRY COMFORT MOTEL 86 BARNEY ST, ARMIDALE NSW 2350'
    incidentlocators.INCIDENT_LOCATION().type(loc_address,{force: true}).wait(5000).type('{downArrow}').type('{enter}')
    incidentlocators.ZOOMBUTTON_INC_MAP().click({force: true})
    ...
    incidentlocators.TRIPLE0_NUMBER().then(($tripleZeroNumberField)=>{
        ...
        cy.wrap(UniqueNumber).as('tripple0')
    ...
    incidentlocators.AGENCY_CALL_RECEIVED().scrollIntoView().then(($agencyReceivedDateTimeField)=>{
        const agencyReceivedDateTimeField = $agencyReceivedDateTimeField
        const eventstartdate = dayjs().subtract(10,'minute').format('DD/MM/YYYY HH:mm');
        cy.wrap(eventstartdate).as('startDate')
        cy.wrap(agencyReceivedDateTimeField).type(eventstartdate, {force: true})
    })
    // I select Other  Fire Type on create full incident page
    incidentlocators.INC_OTHERFIRE_TYPE().select('Chimney fire',{force:true})
...
Then('I should see the created incident displayed on Landing Page map with dotted border',()=>{
    cy.wait(4000)
    cy.get('.leaflet-pane>.leaflet-marker-icon').eq(0).should('be.visible')
    cy.get('@incId').then((incId)=>{
        const id = incId
        incidentlocators.SITREP_PENDING_INC_MARKER_ICON_MAP(id).should('be.visible').click({force:true})
        cy.wait(1000)
    })
})

And('I verify incident details on the popup',()=>{
    incidentlocators.INC_DET_POPUP().should('be.visible')
    cy.get('@incName').then((incName)=>{
        incidentlocators.INC_DET_POPUP_ITEMS().eq(0).invoke('text').should('equal', incName)
    })
cy.get('@lga').then((LGA)=>{
    incidentlocators.INC_DET_POPUP_ITEMS().eq(1).invoke('text').should('equal', LGA)
    })
incidentlocators.INC_DET_POPUP_ITEMS().eq(2).invoke('text').should('equal', 'Pending')
})

Then('I click authorise incident button',()=>{
    incidentlocators.BTN_INC_DET_POPUP_AUTH().should('be.visible').click({force:true})
    cy.url().should('contain','Incidents/SitReps/Details/')
    incidentlocators.BTN_PENDING_SIT_AUTH().should('be.visible')
})

And('I verify details on pending sitrep page',()=>{

    incidentlocators.REF_SITREP().invoke('text').then((sitRR)=>{
        const sitRepRef = sitRR
        cy.wrap(sitRepRef).as('sitRepRef')
    })

    cy.get('@incName').then((name)=>{
        incidentlocators.PENDING_SIT_DET().eq(1).invoke('text').should('equal', name)
    })
    cy.get('@incidentLocation').then((loc)=>{
        incidentlocators.PENDING_SIT_DET().eq(2).invoke('text').should('equal', loc_address)
    })
    cy.get('@lga').then((lga)=>{
        incidentlocators.PENDING_SIT_DET().eq(3).invoke('text').should('contain', lga)
    })
    cy.get('@tripple0').then((t0)=>{
        incidentlocators.PENDING_SIT_DET().eq(5).invoke('text').should('equal', t0)
    })
})

And('I authorise by clicking authorise sitrep',()=>{
    incidentlocators.BTN_PENDING_SIT_AUTH().click({force:true})
    cy.url().should('contain','Incidents/SitReps')
    incidentlocators.INC_IAPAUTHORISESUCCESS().should('be.visible')
    cy.get('@sitRepRef').then((sRR)=>{
        incidentlocators.INC_IAPAUTHORISESUCCESS().invoke('text').should('contain', sRR)
    })
})

Then('I verify the incident id no more in pending incident list',()=>{
    cy.get('@incId').then((incID:any)=>{
        incidentlocators.TXT_SRCH_TAB_INCID().type(incID)
    })
    cy.wait(6000)
    incidentlocators.PENDING_TAB_ROW1().invoke('text').should('equal','No matching records found')
})

Then('I verify on ICON landing page the incident appars as authorised incident',()=>{
    cy.wait(45000)
    ...
    cy.get('@incId').then((incId)=>{
        const id = incId
        incidentlocators.SITREP_NOTAPP_INC_MARKER_ICON_MAP(id).should('be.visible')
        incidentlocators.SITREP_PENDING_INC_MARKER_ICON_MAP(id).should('not.exist')
    })
...
And('I create event with a future date with planned status',()=>{
    ...
    eventlocators.INCIDENT_NAME().then(($nameField)=>{
        const nameField = $nameField
        const timeStamp = Date.now().toString()
        const name = 'ICONSUPT425-T404 Event'+ timeStamp
    ...
    loc_address = '260 WATERLOO RD, WOODSTOCK'
    incidentlocators.INCIDENT_LOCATION().type(loc_address,{force: true}).wait(8000).type('{downArrow}').type('{enter}')
    cy.wait(2000)
    incidentlocators.INC_LOCATION_BUSH2().click({force: true})
...
And ('I verify previous alert level',()=>{
    iall.crossverify_prev_alert_inc_iall('EW (WA)')
})
